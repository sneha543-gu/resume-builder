...
export default function PhotoAmber({ data }: Props) {
  ...
  const AMBER_MEDIUM = '#92400e';
  const GOLD = '#fcd34d';
  const TEXT_DARK = '#111827';

  const SectionHeader = ({ title }: { title: string }) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, margin: '22px 0 12px' }}>
      <span style={{ fontSize: 15, fontWeight: 800, color: AMBER_MEDIUM, letterSpacing: '0.12em', textTransform: 'uppercase' }}>{title}</span>
      <div style={{ flex: 1, height: 1.5, background: '#fef3c7' }} />
    </div>
  );

  const SidebarHeader = ({ title }: { title: string }) => (
    <div style={{ borderBottom: '1px solid rgba(255, 255, 255, 0.25)', paddingBottom: 4, margin: '20px 0 10px' }}>
      <span style={{ fontSize: 13.5, fontWeight: 700, color: GOLD, letterSpacing: '0.1em', textTransform: 'uppercase' }}>{title}</span>
    </div>
  );
  ...
  return (
    <div className="resume-template" style={{ width: 794, minHeight: 1123, fontFamily: '"Segoe UI", sans-serif', background: '#fff', boxSizing: 'border-box', display: 'flex' }}>
      {/* Left Sidebar */}
      <div style={{ width: '34%', background: AMBER_DARK, color: '#fff', padding: '36px 20px', boxSizing: 'border-box', display: 'flex', flexDirection: 'column' }}>
        {/* Photo Container */}
        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 20 }}>
          <div style={{ width: 130, height: 130, borderRadius: '50%', border: `3px solid ${GOLD}`, overflow: 'hidden', background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 6px rgba(0,0,0,0.15)' }}>
            {p.profilePhoto ? (
        ...
        <div style={{ textAlign: 'center', marginBottom: 20 }}>
          ...
          <p style={{ fontSize: 15, color: GOLD, fontWeight: 600, margin: '4px 0 0 0', textTransform: 'uppercase', letterSpacing: 0.5 }}>{p.jobTitle || 'Professional Title'}</p>
        </div>
        ...
        {skills.length > 0 && (
          <>
            ...
            <ul style={{ margin: 0, paddingLeft: 12, fontSize: 13.5, color: '#fef3c7', lineHeight: 1.6 }}>
              {skills.map((s, i) => (
          ...
          <>
            ...
            {education.map((e, i) => (
              <div key={i} style={{ marginBottom: 12 }}>
                ...
                <div style={{ fontSize: 13, color: '#fef3c7', marginTop: 1 }}>{e.college}</div>
        ...
        {workExperience.length > 0 && (
          <div>
            ...
            {workExperience.map((exp, i) => (
              <div key={i} style={{ marginBottom: 16, borderLeft: `3px solid ${AMBER_MEDIUM}`, paddingLeft: 12 }}>
                ...
                <div style={{ fontSize: 14.5, color: AMBER_MEDIUM, fontWeight: 600, fontStyle: 'italic', marginBottom: 5 }}>{exp.company}</div>
              ...
              <div key={i} style={{ marginBottom: 12 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 2 }}>
                  <div style={{ fontWeight: 700, fontSize: 15, color: '#111827' }}>
                    {proj.name} {proj.link && <a href={proj.link} style={{ fontSize: 12.5, color: AMBER_MEDIUM, textDecoration: 'none' }}>↗</a>}
                  </div>
                  {proj.techStack && <div style={{ fontSize: 13.5, color: AMBER_MEDIUM, fontStyle: 'italic' }}>{proj.techStack}</div>}
                </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              {certifications.map((c, i) => (
                <div key={i} style={{ fontSize: 13.5, color: TEXT_DARK }}>
                  <div style={{ fontWeight: 600, color: AMBER_MEDIUM }}>{c.name}</div>
