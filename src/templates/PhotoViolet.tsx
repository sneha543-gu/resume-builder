...
export default function PhotoViolet({ data }: Props) {
  ...
  const SectionHeader = ({ title }: { title: string }) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, margin: '22px 0 12px' }}>
      <span style={{ fontSize: 15, fontWeight: 800, color: VIOLET, letterSpacing: '0.12em', textTransform: 'uppercase' }}>{title}</span>
      <div style={{ flex: 1, height: 1.5, background: '#e9d5ff' }} />
    </div>
  );

  const SidebarHeader = ({ title }: { title: string }) => (
    <div style={{ borderBottom: '1px solid rgba(255, 255, 255, 0.25)', paddingBottom: 4, margin: '20px 0 10px' }}>
      <span style={{ fontSize: 13.5, fontWeight: 700, color: '#c4b5fd', letterSpacing: '0.1em', textTransform: 'uppercase' }}>{title}</span>
    </div>
  );
  ...
  return (
    <div className="resume-template" style={{ width: 794, minHeight: 1123, fontFamily: '"Segoe UI", sans-serif', background: '#fff', boxSizing: 'border-box', display: 'flex' }}>
      {/* Left Sidebar */}
      <div style={{ width: '35%', background: VIOLET, color: '#fff', padding: '36px 20px', boxSizing: 'border-box', display: 'flex', flexDirection: 'column' }}>
        {/* Photo Container */}
        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 20 }}>
          <div style={{ width: 125, height: 125, borderRadius: '50%', border: '3px solid #fff', overflow: 'hidden', background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 6px rgba(0,0,0,0.15)' }}>
            {p.profilePhoto ? (
        ...
        <div style={{ textAlign: 'center', marginBottom: 20 }}>
          ...
          <p style={{ fontSize: 15, color: '#c4b5fd', fontWeight: 600, margin: '4px 0 0 0', textTransform: 'uppercase', letterSpacing: 0.5 }}>{p.jobTitle || 'Professional Title'}</p>
        </div>
        ...
        {workExperience.length > 0 && (
          <div>
            ...
            {workExperience.map((exp, i) => (
              <div key={i} style={{ marginBottom: 16, borderLeft: `3px solid ${VIOLET}`, paddingLeft: 12 }}>
                ...
                <div style={{ fontSize: 14.5, color: VIOLET_LIGHT, fontWeight: 600, fontStyle: 'italic', marginBottom: 5 }}>{exp.company}</div>
          ...
          {certifications.length > 0 && (
            <div style={{ flex: 1.2 }}>
              ...
              {certifications.map((c, i) => (
                <div key={i} style={{ marginBottom: 6, fontSize: 13.5, color: TEXT_DARK }}>
                  <div style={{ fontWeight: 600, color: VIOLET }}>{c.name}</div>
          ...
          {languages.length > 0 && (
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 5, fontSize: 13.5, color: TEXT_DARK }}>
                {languages.map((l, i) => (
                  <div key={i}>
                    <strong style={{ color: VIOLET }}>{l.name}</strong> · <span style={{ color: '#111827' }}>{l.proficiency}</span>
                  </div>
                ))}
              </div>
            </div>
